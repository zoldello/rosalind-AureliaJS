...
(function(process) {
  var gulp = require("gulp-param")(require("gulp"), process.argv),
      ...
      headerText = '/* ' + package.name + ' ' + package.version + ' | ' + package.license + ' licensed | ' + package.homepage + ' */\n\n';
  ...
  gulp.task('browserify', function() {
    return gulp.src(SRC).pipe(browserify({
      detectGlobals: true,
      standalone: 's'
    })).pipe(header(headerText)).pipe(rename('underscore.string.js')).pipe(gulp.dest(DEST));
  });
  ...
  gulp.task('build', ['test', 'clean'], function() {
    gulp.src(DEST + '/' + SRC_COMPILED).pipe(uglify()).pipe(header(headerText)).pipe(rename(MIN_FILE)).pipe(gulp.dest(DEST));
  });
})(require("process"));
